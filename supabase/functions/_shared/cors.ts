...
/** 로컬 네트워크 IP 패턴 (개발 환경 전용: 192.168.x.x, 10.x.x.x, 172.16-31.x.x) */
const LOCAL_NETWORK_RE = /^http:\/\/(192\.168\.\d{1,3}\.\d{1,3}|10\.\d{1,3}\.\d{1,3}\.\d{1,3}|172\.(1[6-9]|2\d|3[01])\.\d{1,3}\.\d{1,3})(:\d+)?$/;

const ALLOW_LOCAL_CORS = Deno.env.get("CORS_ALLOW_LOCAL") === "true";
...
export function getCorsHeaders(req: Request): Record<string, string> {
  ...
  const allowed =
    ...
    (ALLOW_LOCAL_CORS && LOCAL_NETWORK_RE.test(origin));
