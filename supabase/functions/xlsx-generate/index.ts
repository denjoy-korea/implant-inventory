...
Deno.serve(async (req: Request) => {
  ...
  if (typeof parsedBody === "string") {
    try {
      ...
      return jsonError("Body string is not valid JSON", 400);
    }
  ...
  if (!activeSheet || typeof activeSheet !== "object") {
    return jsonError("activeSheet is required", 400);
  }
  if (!Array.isArray(activeSheet.rows)) {
    return jsonError("activeSheet.rows must be an array", 400);
  }
  if (!Array.isArray(activeSheet.columns)) {
    return jsonError("activeSheet.columns must be an array", 400);
  }
  if (!Array.isArray(selectedIndices)) {
    return jsonError("selectedIndices must be an array", 400);
  }
