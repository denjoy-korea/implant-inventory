-- Fix: process_payment_callback의 JWT role 체크 제거
--
-- 문제: current_setting('request.jwt.claim.role', true)이 Supabase PostgREST에서
--       개별 GUC로 설정되지 않아 항상 NULL을 반환 → RETURN FALSE → 409 Conflict
--
-- 근거: GRANT가 이미 service_role 전용으로 설정되어 있으므로 (20260305010000),
--       함수 내부 role 체크는 불필요하며 오히려 정상 결제를 차단하는 버그가 됨.
