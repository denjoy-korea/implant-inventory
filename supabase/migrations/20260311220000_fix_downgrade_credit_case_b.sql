-- Fix: execute_downgrade_with_credit Case B 잔여일 계산 버그 수정
--
-- 버그:
--   Case B (2차+ 다운그레이드)에서 plan_expires_at 기반 remaining_days 계산 시
--   달력 31일 짜리 달의 경우 remaining_days(31) > total_days(30)이 되어
--   크레딧이 플랜 가격(29,700원)을 초과(30,690원)하는 문제.
--
--   또한 plan_changed_at을 무시하여 오늘 이미 사용한 일수가 차감되지 않는 문제.
--
-- 수정:
--   plan_changed_at 컬럼을 조회하여 사용 일수를 직접 계산.
--   (Case A와 동일한 방식: usedDays = CEIL(now - plan_changed_at))
--   upper_remaining은 currentPlanVat를 상한으로 캡.
