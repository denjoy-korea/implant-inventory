-- 다운그레이드 크레딧 상한 버그 수정 (2차)
--
-- 버그 1: Business→Plus→Basic→Free 순서로 다운그레이드 시 크레딧 누적 초과
-- 버그 2: 크레딧으로 재구독 후 다운그레이드 시 크레딧 재발행 (무한반복 가능)
--   예) Business 141,900 결제 → 다운그레이드 → 크레딧 141,900
--       크레딧으로 Business 재구독 (실결제 0원) → 다운그레이드 → 크레딧 141,900 또 발행
--
-- 수정:
--   billing_history.credits_issued: 해당 결제에서 발행된 총 크레딧 추적
--   credit_ceiling = amount - credit_used_amount - upgrade_credit_amount (실현금 납입액 기준)
--   → 크레딧/업그레이드크레딧으로 결제한 구독은 다운그레이드 크레딧 발행 불가

-- 1. billing_history에 크레딧 발행 추적 컬럼 추가
ALTER TABLE billing_history
  ADD COLUMN IF NOT EXISTS credits_issued NUMERIC DEFAULT 0;

-- 2. execute_downgrade_with_credit 함수 업데이트 (크레딧 상한 적용)
...
BEGIN
  ...
  IF FOUND AND v_billing.amount > 0 THEN
    ...
    -- (크레딧/업그레이드크레딧으로 결제한 부분은 상한에서 제외)
