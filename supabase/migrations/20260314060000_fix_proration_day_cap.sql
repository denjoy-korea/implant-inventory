-- ────────────────────────────────────────────────────────────────────────────
-- [M-1] execute_downgrade_with_credit — 30/360 관례 vs. 달력일 불일치 방어
--
-- 문제: Case A에서 v_total_days=30(월간), v_used_days=31(1월 31일 결제)일 때
--       v_used_days >= v_total_days → v_upper_remaining = 0 → 크레딧 없음
--       그러나 plan_expires_at > now()이면 구독이 아직 유효 → 크레딧 0은 부당
-- 수정: Case A usedDays 계산 직후, plan_expires_at > now()이면
--       v_used_days := v_total_days - 1 로 캡 (잔여 1일 크레딧 보장)
-- ────────────────────────────────────────────────────────────────────────────
...
BEGIN
  ...
  IF FOUND AND v_billing.amount > 0 THEN
    ...
    IF v_billing.plan = v_hospital_plan THEN
      ...
      -- [M-1] 30/360 관례 vs. 달력일 불일치 보호:
      -- 구독이 아직 유효(plan_expires_at > now())한데 usedDays >= totalDays이면
      -- (예: 1월 31일 결제 → 31일 경과, total_days=30) 잔여 1일로 캡
      IF v_used_days >= v_total_days
         AND v_current_expiry IS NOT NULL
         AND v_current_expiry > now()
      THEN
        v_used_days := v_total_days - 1;
      END IF;
