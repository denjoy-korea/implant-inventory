# Data Room Index

- 작성일: 2026-03-04
- 최종 업데이트: 2026-03-05
- 목적: 투자/파트너 검토 시 필요한 증빙의 위치와 최신 상태를 단일 문서에서 확인

## 01-contracts
- 계약 현황: `docs/05-dataroom/01-contracts/contract-list.md` ✅ (2026-03-05 — 클릭-스루 SaaS, 유료 0건)
- 계약 증빙 레저: `docs/05-dataroom/01-contracts/2026-03-05_contract-evidence-ledger_v1.md` ✅

## 02-billing-reconciliation
- 청구/수금 대사: `docs/05-dataroom/02-billing-reconciliation/billing-reconciliation-2026-03.md` ✅ (2026-03-05 — 테스트 모드, 가격/플랜 정합성 반영)
- 청구/수금 증빙 레저: `docs/05-dataroom/02-billing-reconciliation/2026-03-05_billing-evidence-ledger_v1.md` ✅
- 대사 템플릿: `docs/05-dataroom/02-billing-reconciliation/billing-reconciliation-template.csv`

## 03-refund-termination
- 환불/해지 로그: `docs/05-dataroom/03-refund-termination/refund-termination-log.md` ✅ (2026-03-05 — 0건)
- 환불/해지 증빙 레저: `docs/05-dataroom/03-refund-termination/2026-03-05_refund-termination-evidence-ledger_v1.md` ✅

## 04-security-operations
- verify:premerge 3회 연속 Green: `docs/05-dataroom/04-security-operations/verify-premerge-log-2026-03-05.md` ✅
- verify:release Green: `docs/05-dataroom/04-security-operations/verify-release-log-2026-03-05.md` ✅
- 보안 운영 주간 업데이트: `docs/05-dataroom/04-security-operations/2026-03-05_security-operations-weekly-update_v1.md` ✅
- 보안 운영 증빙 최신화 패키지: `docs/05-dataroom/04-security-operations/2026-03-05_security-operations-evidence-refresh_v1.md` ✅
- 이전 보안 운영 증빙: `docs/05-dataroom/04-security-operations/2026-03-04_*`

## 05-policy-versioning
- 이용약관 v1.0: `docs/05-dataroom/05-policy-versioning/2026-03-04_terms-of-service_v1.md` ✅
- 개인정보 처리방침 v1.0: `docs/05-dataroom/05-policy-versioning/2026-03-04_privacy-policy_v1.md` ✅
- 환불정책 v1.0: `docs/05-dataroom/05-policy-versioning/2026-03-04_refund-policy_v1.md` ✅
- 법무 패키지 종료 문서: `docs/05-dataroom/05-policy-versioning/2026-03-05_legal-package-closeout_v1.md` ✅

## 06-investor-pack
- 투자자 패키지 인덱스: `docs/05-dataroom/06-investor-pack/2026-03-04_investor-pack-index_v1.md` ✅ (v1.5, 2026-03-05)
- 비식별 작업 체크리스트: `docs/05-dataroom/06-investor-pack/redacted/redaction-prep-checklist-2026-03-05.md` ✅
- I-02 레드액트 공유본: `docs/05-dataroom/06-investor-pack/redacted/2026-03-05_investor-pack-redacted-summary_v1.md` ✅
- Legal/Finance 검토 로그: `docs/05-dataroom/06-investor-pack/redacted/2026-03-05_legal-finance-review-round_v1.md` ✅
- I-01 원본 ZIP: `docs/05-dataroom/06-investor-pack/original/DenJOY-DataRoom-v1-2026-03-05.zip` ✅
- I-01 SHA256: `docs/05-dataroom/06-investor-pack/original/DenJOY-DataRoom-v1-2026-03-05.zip.sha256` ✅
- 내부 원본 폴더: `docs/05-dataroom/06-investor-pack/original/`
- 외부 공유본: `docs/05-dataroom/06-investor-pack/redacted/`

## 99-index
- 체크리스트: `docs/05-dataroom/99-index/dataroom-checklist.md`
- 실행 추적: `docs/05-dataroom/execution-tracker-2026-03-04.md`
- 증빙 수집 체크리스트: `docs/05-dataroom/evidence-collection-checklist-2026-03-04.md`
