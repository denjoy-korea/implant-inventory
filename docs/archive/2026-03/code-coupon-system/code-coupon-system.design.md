...
**파일**: `supabase/migrations/20260306210000_create_coupon_tables.sql`
**적용 방식**: Supabase MCP로 직접 DB 적용 (마이그레이션 파일은 stub 참조용)
...
export interface CouponStats {
  total: number; active: number; exhausted: number; expired: number; revoked: number; totalUsed: number;
}

export interface RedemptionStats {
  totalRedemptions: number; totalDiscountAmount: number;
}

export interface ChannelStat {
  channel: string; totalCodes: number; activeCodes: number; totalVerifications: number; signups: number;
}
...
export const couponService = {
  ...
  async getCouponStats(): Promise<CouponStats> { /* user_coupons 집계: total, active, exhausted, expired, revoked, totalUsed */ },
  async getRedemptionStats(): Promise<RedemptionStats> { /* coupon_redemptions 집계: totalRedemptions, totalDiscountAmount */ },
  async getChannelStats(): Promise<ChannelStat[]> { /* beta_invite_codes(partner) 집계: channel, totalCodes, activeCodes, totalVerifications, signups */ },
};
```
...
// 5. 결제 성공 후 쿠폰 차감
// 구현: process_payment_callback RPC 내부에서 redeem_coupon을 원자적으로 호출
// (Edge Function에서 별도 RPC 호출 대신, DB 트랜잭션 내에서 일괄 처리)
// → billing_history coupon_id가 존재하면 process_payment_callback이
//   redeem_coupon(coupon_id, user_id, hospital_id, billing_id, billing_cycle, original_amount) 호출
```
...
```typescript
// 별도 필드 대신 조건부 렌더링으로 구현:
// !isBetaInviteRequired && !betaInviteVerified → 선택적 코드 입력란 표시
// AuthSignupDentistScreen, AuthSignupStaffScreen에서 동일 패턴 사용
```
