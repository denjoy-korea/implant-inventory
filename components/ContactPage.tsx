...
const TESTIMONIALS = [
  {
    quote: '수술 기록과 재고가 자동으로 연동되니까 어떤 사이즈가 부족한지 미리 알 수 있어서 발주 실수가 확 줄었어요.',
    initial: '박',
    name: '박OO 실장',
    clinic: '경기 · 치과의원',
  },
  {
    quote: '여러 브랜드 재고를 한 곳에서 관리할 수 있게 됐습니다. 브랜드별 소모 패턴이 달라서 정말 유용해요.',
    initial: '이',
    name: '이OO 매니저',
    clinic: '부산 · 치과의원',
  },
  {
    quote: '엑셀로 하루 종일 걸리던 작업이 3초면 끝납니다. 직원들 업무 부담이 크게 줄었어요.',
    initial: '최',
    name: '최OO 원장',
    clinic: '인천 · 치과의원',
  },
...
const FAQ_ITEMS = [
  {
    q: '상담 비용이 있나요?',
    a: '첫 상담은 무료입니다. 현재 상황과 도입 목적을 먼저 듣고, 필요한 범위만 안내합니다.',
  },
  {
    q: '답변까지 얼마나 걸리나요?',
    a: '영업일 기준 24시간 이내 답변을 목표로 합니다. 대부분의 경우 당일 또는 다음 영업일에 회신합니다.',
  },
  {
    q: '어떤 내용을 문의할 수 있나요?',
    a: '도입 상담, 요금제, 데이터 이관, 맞춤 기능, 운영 방식까지 모두 문의하실 수 있습니다.',
  },
  {
    q: '개인정보는 어떻게 사용되나요?',
    a: '문의 응대와 후속 안내에만 사용되며, 목적 달성 후 내부 정책에 따라 안전하게 관리합니다.',
  },
] as const;
...
const ContactPage: React.FC<ContactPageProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!validateEmail(form.email)) {
      showToast('이메일 형식이 올바르지 않습니다. (예: name@naver.com)', 'error');
      return;
    }
    if (!form.agree) {
  ...
  return (
    <>
      <div className="min-h-screen bg-mesh overflow-x-hidden flex flex-col font-sans selection:bg-indigo-500 selection:text-white">
        <div className="orb orb-purple w-[520px] h-[520px] -top-44 -left-40 animate-pulse-glow" aria-hidden="true" />
        <div className="orb orb-blue w-[420px] h-[420px] top-1/4 -right-32 animate-pulse-glow" aria-hidden="true" style={{ animationDelay: '1s' }} />
        ...
        <main className="flex-1 relative z-10 pt-20">
          <section className="relative py-16 md:py-24">
            <div className="hidden md:block absolute top-10 left-1/2 -translate-x-1/2" aria-hidden="true">
              <div className="w-8 h-8 rounded-full bg-indigo-100 border-2 border-indigo-200 opacity-60" />
            </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-start">
                <div className="pt-2 lg:pt-4">
                  ...
                  <div className="flex flex-wrap items-center gap-3 mb-8">
                    {[
                      { step: '1', label: '문의 작성' },
                      { step: '2', label: '24시간 내 답변' },
                      { step: '3', label: '도입 방향 안내' },
                    ].map((item, index) => (
                      <div key={item.step} className="flex items-center gap-2">
                        ...
                        {index < 2 && (
                          <svg aria-hidden="true" className="w-4 h-4 text-slate-300 ml-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  <div className="flex flex-wrap gap-3 mb-8">
                    {[
                      ...
                      <div key={text} className="flex items-center gap-1.5 text-sm text-emerald-600">
                  ...
                  <p className="text-slate-300 font-black text-lg tracking-tight mb-10 leading-relaxed">
                    OSSTEM · Dentium · Megagen · Neobiotech · DIO · Warantec · Dentis · Straumann · Magicore · 신흥 · 탑플란 · 포인트임플란트 등
                  </p>

                  <div className="bg-white/75 backdrop-blur-sm rounded-2xl p-6 border border-slate-200 shadow-sm">
                    ...
                    <p className="text-slate-600 leading-relaxed mb-5">
                      "문의 후 하루 만에 세팅 완료됐습니다. 덴트웹 데이터 정리에 매주 2시간씩 쓰던 시간이 사라졌고, 재고 파악이 한눈에 되니까 발주 실수도 확 줄었어요."
                    </p>
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-slate-400">서울 · 치과의원</p>
                      </div>
                    </div>
                  </div>
                </div>

                <div className="bg-white/90 backdrop-blur-sm rounded-2xl border border-slate-200 p-6 sm:p-8 shadow-xl shadow-slate-200/40">
                  {submitted ? (
                    <div>
          ...
          <section className="py-10 border-y border-slate-200/70 bg-white/70 backdrop-blur-sm">
            <div className="max-w-4xl mx-auto px-6 lg:px-8">
              <div className="flex flex-wrap items-center justify-center gap-8 text-center">
                {[
                  { value: '24시간 이내', label: '빠른 답변' },
                  { value: '1:1 맞춤', label: '전문 상담' },
                  { value: '맞춤 방향', label: '도입 안내' },
                ].map((item) => (
            ...
            <div className="max-w-6xl mx-auto px-6 lg:px-8">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                {TESTIMONIALS.map((item) => (
                  <div key={item.name} className="rounded-2xl border border-slate-200 bg-white/80 backdrop-blur-sm p-6">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-slate-400">{item.clinic}</p>
                      </div>
                    </div>
                  </div>
                ))}
          ...
          <section className="py-16 md:py-20 border-t border-slate-200/70 bg-white/80">
            <div className="max-w-3xl mx-auto px-6 lg:px-8">
              <h2 className="text-2xl font-bold text-slate-900 text-center mb-10">자주 묻는 질문</h2>
              <div className="space-y-4">
                {FAQ_ITEMS.map((item) => (
                  <div key={item.q} className="bg-slate-50 rounded-xl p-5 border border-slate-100">
