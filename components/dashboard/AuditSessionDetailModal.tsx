...
const AuditSessionDetailModal: React.FC<AuditSessionDetailModalProps> = ({
  ...
  const sessionItems = selectedAuditSessionKey
    ? auditHistory.filter((row) => {
  ...
  return (
    <ModalShell
      isOpen={selectedAuditSessionKey !== null}
      onClose={onClose}
      title="재고 실사 상세"
      maxWidth="max-w-2xl"
      className="flex flex-col max-h-[calc(100dvh-80px)] md:max-h-[85vh]"
    >
      {/* Header */}
