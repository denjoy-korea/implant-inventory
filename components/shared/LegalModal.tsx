...
const TERMS_SECTIONS: Array<{ title: string; paragraphs: ParagraphEntry[] }> = [
  ...
  },
  {
    ...
    paragraphs: [
      ...
      {
        text: '유료 플랜 이용 중 상위 플랜으로 업그레이드 시, 기존 구독의 잔여 금액이 일할 계산 방식으로 산출되어 신규 결제금액에서 차감됩니다.',
        highlight: true,
      },
  ...
  },
  {
    ...
    paragraphs: [
      ...
      {
        text: '(환불 산정) 환불 금액은 결제 금액에서 이용 일수에 해당하는 금액을 공제하여 산정합니다. 일일 사용료는 결제금액을 기준일(월간 30일, 연간 360일)로 나누어 10원 단위로 올림 처리하며, 업그레이드 시 잔여 크레딧도 동일한 방식으로 산출됩니다.',
        highlight: true,
      },
      '(연간 구독) 연간 구독도 월간 구독과 동일하게 360일 기준 일할 계산을 적용합니다.',
