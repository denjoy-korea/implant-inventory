...
const StaffWaitingRoom: React.FC<StaffWaitingRoomProps> = ({ currentUser, onUpdateUser, onLogout }) => {
    ...
    const modals = (
        <>
            ...
            {toast && (
                <div className={`fixed bottom-6 left-1/2 -translate-x-1/2 z-50 px-5 py-3 rounded-2xl shadow-xl text-sm font-semibold flex items-center gap-2 ${toast.type === 'success' ? 'bg-emerald-600 text-white' : 'bg-rose-600 text-white'}`}>
                    {toast.message}
                </div>
            )}
        </>
