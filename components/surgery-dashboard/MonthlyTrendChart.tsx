...
export default function MonthlyTrendChart({ monthlyData, mounted }: Props) {
          ...
          {monthlyData.map((d, i) => {
            ...
            return (
              <g key={d.month}>
                ...
                {isHovered && (
                  <rect x={pad.l + i * groupW + 4} y={pad.t - 4} width={groupW - 8} height={plotH + 20} rx={6} fill="#F1F5F9" />
                )}
                {isHovered && (
                  <line x1={groupX} y1={pad.t} x2={groupX} y2={pad.t + plotH} stroke="#CBD5E1" strokeWidth={1} strokeDasharray="4 4" />
                )}
                {BAR_SERIES.map((s, si) => {
                  const val = d[s.key];
                ...
                <text x={groupX} y={H - pad.b + 24} textAnchor="middle"
                  fontSize={10} fontWeight={isHovered ? 500 : 400} fill={isHovered ? '#334155' : '#94a3b8'}>
                  {d.month.substring(2)}
                </text>
              </g>
            );
