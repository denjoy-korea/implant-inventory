...
const PERM_GROUPS: { label: string; keys: (keyof MemberPermissions)[] }[] = [
    ...
    {
        label: '설정 권한',
        keys: ['canManageVendors', 'canManageWorkDays', 'canManageOptimizer', 'canViewAuditLog', 'canManageDentweb'],
    },
];
...
const PermissionModal: React.FC<PermissionModalProps> = ({ member, onClose, onSave }) => {
    ...
    const storedPerms = member.permissions as (MemberPermissions & { canManageVendors?: boolean; canManageWorkDays?: boolean; canManageOptimizer?: boolean; canViewAuditLog?: boolean; canManageDentweb?: boolean }) | null;
    const initialPerms: MemberPermissions = storedPerms
        ? { ...storedPerms, canManageVendors: storedPerms.canManageVendors ?? false, canManageWorkDays: storedPerms.canManageWorkDays ?? false, canManageOptimizer: storedPerms.canManageOptimizer ?? false, canViewAuditLog: storedPerms.canViewAuditLog ?? false, canManageDentweb: storedPerms.canManageDentweb ?? false }
