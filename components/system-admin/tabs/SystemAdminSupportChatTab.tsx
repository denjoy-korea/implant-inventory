...
import type { SupportAlertSnapshot } from '../../../services/adminSupportAlertService';
import {
...
interface SystemAdminSupportChatTabProps {
  ...
  onEnableSupportAlerts: () => void;
  onDisableSupportAlerts: () => void;
}
...
const SystemAdminSupportChatTab: React.FC<SystemAdminSupportChatTabProps> = ({
  ...
  onClearSelectedSupportThread,
  onSupportDraftChange,
  onSendSupportMessage,
  onUpdateSupportThreadStatus,
  onEnableSupportAlerts,
  onDisableSupportAlerts,
}) => {
  ...
  const alertsEnabled = supportAlertState.permission === 'granted' && supportAlertState.enabled;
  const alertsUnsupported = supportAlertState.permission === 'unsupported';
  const alertButtonLabel = alertsUnsupported ? '브라우저 미지원' : alertsEnabled ? '알림 켜짐' : '알림 켜기';
  const showThreadListPanel = !isMobileViewport || !selectedSupportThread;
  const showConversationPanel = !isMobileViewport || Boolean(selectedSupportThread);
  ...
  return (
    <div className="flex h-full min-h-0 flex-col space-y-4 overflow-hidden">
        ...
        <div className="flex flex-col gap-3">
          <div className="flex flex-wrap items-center justify-between gap-2">
            ...
            <button
              ...
              onClick={alertsEnabled ? onDisableSupportAlerts : onEnableSupportAlerts}
            ...
            >
              <span className={`flex h-8 w-8 items-center justify-center rounded-full ${
                alertsUnsupported
                  ...
                  : alertsEnabled ? 'bg-emerald-100 text-emerald-600' : 'bg-white/10 text-white'
              }`}>
                <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" />
                </svg>
              </span>
              <span>{alertButtonLabel}</span>
            </button>
          </div>

          {isMobileViewport && (
            <div className="rounded-[1.4rem] border border-slate-200 bg-white/90 px-4 py-3 text-[12px] leading-5 text-slate-500">
              <p className="font-bold text-slate-700">스마트폰 응답 안내</p>
              <p className="mt-1">브라우저 알림을 켜고 홈 화면에 추가하면 새 문의를 더 안정적으로 받을 수 있습니다.</p>
              {!supportAlertState.isStandalone && (
                <p className="mt-1.5 text-[11px] text-slate-400">iPhone은 Safari에서 공유 버튼 → 홈 화면에 추가 후 알림 허용이 필요합니다.</p>
              )}
            </div>
          )}
        </div>
      ...
      {supportThreadsLoading ? (
        <div className="flex flex-1 items-center justify-center rounded-[2rem] border border-slate-200 bg-white py-20 text-center text-sm text-slate-400">
          상담방을 불러오는 중...
        </div>
      ) : supportThreads.length === 0 ? (
        <div className="flex flex-1 items-center justify-center rounded-[2rem] border border-dashed border-slate-300 bg-white">
        ...
        <div className={`grid min-h-0 flex-1 gap-4 ${isMobileViewport ? 'grid-cols-1' : 'xl:grid-cols-[300px_minmax(0,1fr)]'}`}>
          {showThreadListPanel && (
            <aside className="flex min-h-0 flex-col rounded-[2rem] border border-slate-200 bg-[linear-gradient(180deg,#ffffff,#f8fafc)] p-3 shadow-sm">
              <div className="mb-3 flex items-center justify-between px-1">
                <div>
                  <p className="text-xs font-black uppercase tracking-[0.2em] text-slate-400">Support Inbox</p>
                  <p className="mt-1 text-sm font-bold text-slate-900">회원 문의 목록</p>
                </div>
                {isMobileViewport && selectedSupportThread && (
                  <button
                    type="button"
                    onClick={onClearSelectedSupportThread}
                    className="rounded-full border border-slate-200 px-3 py-1.5 text-[11px] font-bold text-slate-500"
                  >
                    목록
                  </button>
                )}
              ...
              <div className="min-h-0 flex-1 space-y-2 overflow-y-auto pr-1 [content-visibility:auto]">
          ...
          {showConversationPanel ? (
            selectedSupportThread ? (
              <section className="flex min-h-0 flex-col rounded-[2rem] border border-slate-200 bg-[radial-gradient(circle_at_top,rgba(15,23,42,0.05),transparent_48%),linear-gradient(180deg,#fff,#f8fafc)] shadow-sm">
                ...
                <div className="flex min-h-0 flex-1 flex-col">
                  <div className="min-h-0 flex-1 space-y-3 overflow-y-auto px-4 py-5 sm:px-5 [content-visibility:auto]">
                    {supportMessagesLoading ? (
                      ...
                      supportMessages.map((message) => {
                        ...
                        return (
                          <div key={message.id} className={`flex ${isAdmin ? 'justify-end' : 'justify-start'}`}>
                            <div className={`max-w-[90%] rounded-[1.5rem] px-4 py-3 shadow-sm sm:max-w-[86%] ${
                              isAdmin
