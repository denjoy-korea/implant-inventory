...
| modal-accessibility (full 3-phase) | 2026-03-06 | 96.3% | 33/33 plan modals migrated to ModalShell. R-01~R-15 all PASS. 13 additional modals also use ModalShell (46 total). 6 files with 9 inline patterns remain (MemberManager, IntegrationManager, AuthForm, OrderHistoryPanel, UserProfile, AnalyzeSizeFormatDetailModal). 1 redundant useEscapeKey in useFailManager.ts. Archival eligible. |
...
| valuation-narrowing | 2026-03-05 | 90.0% (v1.1) | 15 items (WS3:6, WS4:3, WS5:6). WS4 100%. 0 FAIL. FR-W3-01 resolved (quality-check-template.md created). 3 PARTIAL remain: FR-W3-02 28-day snapshots, FR-W5-01-sub mrr-summary blocked, FR-W5-complete investor redaction. 90% threshold reached. |
| traffic-tracking | 2026-03-05 | 96.5% | 57 items, 0 FAIL, 0 PARTIAL. 10 ADDED (positive): account_id, client context, auth/payment/PWA events, event funnel KPI, RLS hardening, SEC-11. 4 CHANGED (neutral): event_type ${page}_view vs NULL, admin-only RLS, hook extraction, migration ordering. Design doc update recommended for additions. |
| tech-debt-remediation | 2026-03-06 | 82.4% | 17 items (P0-P2), 14 PASS, 3 FAIL. FAIL: #1 withdrawal test OR-assertions missing (hardcodes UserProfile.tsx only), #3 supabase/_archive/ not created (3 SQL files still in supabase/ root), #4 fix_plan_max_items_free migration missing. P2 all PASS: useSystemAdminDashboard 104 LOC (5 sub-hooks), InventoryManager 541 LOC, unregisteredMatchingUtils 24 tests. P3 items 18-20 FAIL-PLANNED. |
...
- Admin Hooks: `/hooks/admin/` (adminTypes, useAdminReviews, useAdminManuals, useAdminAnalysisLeads, useAdminContacts, useAdminUsers)
- Inventory Extracted: `/components/inventory/InventoryDashboardCards.tsx` (267 LOC), `/components/inventory/InventoryUsageChart.tsx` (290 LOC)
- Matching Utils: `/services/unregisteredMatchingUtils.ts` (pure functions extracted from UnregisteredDetailModal)
- DentWeb Automation: `/supabase/functions/dentweb-automation/index.ts`, `/supabase/functions/dentweb-upload/index.ts`, `/services/dentwebAutomationService.ts`, `/agent/` (Python agent)
- Coupon System: `/services/couponService.ts`, `/services/betaInviteService.ts` (CodeType extension), `/components/DirectPaymentModal.tsx`, `/components/pricing/PricingPaymentModal.tsx` (coupon dropdown)
